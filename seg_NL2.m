NL_segmented=zeros(7,4,218,182,182,3);
tic
...
for i=1:1:size(NLT,1)
    for j=1:1:4
        ...
        aa(:,:,:)=NLT(i,j,:,:,:);
        NL_segmented(i,j,:,:,:,1)=q1(:,:,:).*aa(:,:,:);
        NL_segmented(i,j,:,:,:,2)=q2(:,:,:).*aa(:,:,:);
        %NL_segmented(i,j,:,:,:,3)=q3(:,:,:).*aa(:,:,:);
        NL_segmented(i,j,:,:,:,3)=q3(:,:,:).*b_whiteT2(:,:,:);
    end
end
toc
